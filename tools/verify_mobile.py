...
with sync_playwright() as p:
    ...
    # ---- 11c. drag feasibility overlay on touch -------------------------
    # Dragging is free-form (the desktop suite covers that); this checks the
    # page-specific wiring: grab starts the overlay, release clears it, a
    # second finger (pinch outranks the drag) clears it too, and the "Drag
    # regions" checkbox in the sheet suppresses it.
    pg.reload(); pg.wait_for_timeout(600); ready(pg)
    ...
    pg.wait_for_timeout(200)
    # the sheet checkbox suppresses it entirely
    pg.evaluate("""() => { const el=document.getElementById('region');
    ...
    print("overlay on touch: starts on grab, clears on release and on pinch, "
          "and the sheet checkbox suppresses it")
